...
mkdir ${Here}/${FILENAME}/count
for(( Tn=0;Tn<${#Temp_seq[@]};Tn++))
do
Temp=${Temp_seq[$Tn]}
cat > ${Here}/${FILENAME}/count/count_${Tn}.m <<EOF
...
writematrix(count, '${Here}/${FILENAME}/count/count_${Tn}.csv');

EOF
#matlab -nosplash -nodesktop -r "run ${Here}/${FILENAME}/count/count_${Tn}.m;clear scriptname;quit;"
done


mkdir ${FILENAME}/WHAM

cat > ${FILENAME}/WHAM/wham.m <<EOF
count='${Here}/${FILENAME}/count';
reweighting(count);
...
function reweighting(count)
    ...
    filename = fullfile('${Here}/${FILENAME}/WHAM','dis_0.list');
    ...
    [n, New_p, A_Q, A_E, cv, F] = nnn('${Here}/${FILENAME}/WHAM',temperatures, dis, Q_num, pot_num);
    
    writematrix(A_Q, fullfile('${Here}/${FILENAME}/WHAM', 'A_Q.csv'));
    writematrix(A_E, fullfile('${Here}/${FILENAME}/WHAM', 'A_E.csv'));    
    writematrix(n, fullfile('${Here}/${FILENAME}/WHAM', 'n.csv'));
    writematrix(cv, fullfile('${Here}/${FILENAME}/WHAM', 'cv.csv'));
    writematrix(F, fullfile('${Here}/${FILENAME}/WHAM', 'F.csv'));
...
EOF
module load matlab2022
#matlab -nosplash -nodesktop -r "run ${FILENAME}/WHAM/wham.m;clear scriptname;quit;"
