...
echo "POT_min: $pot_min"
echo "POT_max: $pot_max"
...
for ionic in 01 05 15 20 25 30
do
...
do
Temp=${Temp_seq[$Tn]}
cat > ${Here}/${FILENAME}/Dih_WHAM/count/count_${Tn}.m <<EOF
...
Q = dlmread('${DIH}/T_${Tn}/dih_all/average_dih.xvg');
...
writematrix(count, '${Here}/${FILENAME}/Dih_WHAM/count/hist_${Tn}.csv');

writematrix(m, '${Here}/${FILENAME}/Dih_WHAM/count/count_${Tn}.csv');
EOF
done

cat > ${Here}/${FILENAME}/Dih_WHAM/WHAM/wham.m <<EOF
count='${Here}/${FILENAME}/Dih_WHAM/count';
reweighting(count);
...
function reweighting(count)
    ...
    filename = fullfile('${Here}/${FILENAME}/Dih_WHAM/WHAM','dis_0.list');
    ...
    [n, New_p, A_Q, A_E, cv, F] = nnn('${Here}/${FILENAME}/Dih_WHAM/WHAM',temperatures, dis, Q_num, pot_num);
    
    writematrix(A_Q, fullfile('${Here}/${FILENAME}/Dih_WHAM/WHAM', 'A_Q.csv'));
    writematrix(A_E, fullfile('${Here}/${FILENAME}/Dih_WHAM/WHAM', 'A_E.csv'));    
    writematrix(n, fullfile('${Here}/${FILENAME}/Dih_WHAM/WHAM', 'n.csv'));
    writematrix(cv, fullfile('${Here}/${FILENAME}/Dih_WHAM/WHAM', 'cv.csv'));
    writematrix(F, fullfile('${Here}/${FILENAME}/Dih_WHAM/WHAM', 'F.csv'));
